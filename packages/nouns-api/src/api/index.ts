...
// /api/nounv2-feed lives in ./activityFeed.ts (registered below with /api/activity).
...
// LIL_NOUNS_SUBGRAPH is shared with the activity feed — see ./activityFeed.ts
...
// Activity-feed external sources (Lil Nouns subgraph, on-chain sale detection)
// moved to ./activityFeed.ts — `checkTxForSale` is imported from there.
...
- Be honest about capabilities. You're an LLM with tool-calling, persistent memory, and access to noun.wtf's Ponder index.`;
    ...
    // The webapp already appends the current message to `history`; don't send it twice.
...
// ─── Activity Feed ──────────────────────────────────────────────────────────
// GET /api/activity + /api/nounv2-feed — see ./activityFeed.ts for the source
// registry, post-processors and the event-type contract.
