// Full ABI for NounV2Treasury, extracted from
// packages/nouns-contracts/contracts/nounv2/NounV2Treasury.sol.
//
// Differences vs SmallGrantsTreasury: adds PROPOSAL_THRESHOLD constant (uint96 = 1)
// and BelowProposalThreshold error. Everything else is a 1:1 fork.
//
// Address is injected via VITE_NOUNV2_TREASURY_ADDRESS after deployment.

export const nounV2TreasuryAbi = [
  {
    type: 'constructor',
    inputs: [
      { name: '_nounsToken', type: 'address', internalType: 'address' },
  ...
  },
  {
    ...
    outputs: [
      ...
      { name: 'queued', type: 'bool', internalType: 'bool' },
    ],
    stateMutability: 'view',
  },
  {
  ...
  // ─── View helpers ──────────────────────────────────────────────────────
  {
    ...
    inputs: [{ name: 'proposalId', type: 'uint256', internalType: 'uint256' }],
    outputs: [
      { name: '', type: 'uint8', internalType: 'enum NounV2Treasury.ProposalState' },
    ],
    stateMutability: 'view',
  },
  {
    ...
    outputs: [
      {
        name: '',
        type: 'tuple',
        internalType: 'struct NounV2Treasury.Receipt',
        components: [
          { name: 'hasVoted', type: 'bool', internalType: 'bool' },
          { name: 'support', type: 'uint8', internalType: 'uint8' },
          { name: 'votes', type: 'uint96', internalType: 'uint96' },
  ...
  { type: 'error', name: 'InvalidProposal', inputs: [] },
  {
    ...
    name: 'InvalidState',
    inputs: [
      { name: 'current', type: 'uint8', internalType: 'enum NounV2Treasury.ProposalState' },
      { name: 'expected', type: 'uint8', internalType: 'enum NounV2Treasury.ProposalState' },
    ],
  },
  { type: 'error', name: 'AlreadyVoted', inputs: [] },
  { type: 'error', name: 'NoVotingPower', inputs: [] },
  { type: 'error', name: 'BelowProposalThreshold', inputs: [] },
  { type: 'error', name: 'NotProposerOrAdmin', inputs: [] },
  { type: 'error', name: 'TimelockNotReady', inputs: [] },
  ...
  { type: 'error', name: 'TooManyOperations', inputs: [] },
] as const;

// ProposalState enum matches NounV2Treasury.sol ordering.
export enum NounV2ProposalState {
  Active = 0,
  Canceled = 1,
  Defeated = 2,
  Succeeded = 3,
...
export const NOUNV2_PROPOSAL_STATE_LABELS: Record<NounV2ProposalState, string> = {
  [NounV2ProposalState.Active]: 'ACTIVE',
  [NounV2ProposalState.Canceled]: 'CANCELED',
  [NounV2ProposalState.Defeated]: 'DEFEATED',
  [NounV2ProposalState.Succeeded]: 'SUCCEEDED',
  [NounV2ProposalState.Queued]: 'QUEUED',
  [NounV2ProposalState.Expired]: 'EXPIRED',
  [NounV2ProposalState.Executed]: 'EXECUTED',
};

export const NOUNV2_TREASURY_ADDRESS = ((import.meta.env.VITE_NOUNV2_TREASURY_ADDRESS as
  | string
