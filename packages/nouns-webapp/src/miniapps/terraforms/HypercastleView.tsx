/* eslint-disable react/no-unknown-property, @typescript-eslint/strict-boolean-expressions, @typescript-eslint/no-explicit-any */
/**
 ...
 * Hover a parcel to preview the live NFT in the bottom-left;
 * click to open a popover modal with the full token + metadata.
 */
...
import ParcelModal, { wrapTokenHTML } from './ParcelModal';
...
export function useHypercastleData() {
  ...
  // Sort by sz descending — top of the Hypercastle first. Combined with the
  // top-down spawn camera + progressive InstancedMesh count reveal in
  // AllParcelsInstanced, this makes the structure appear from the peak down.
  const sortedParcels = useMemo(() => [...parcels].sort((a, b) => b.sz - a.sz), [parcels]);

  return { parcels: sortedParcels, loadedCount, isLoading, total: TOTAL_SUPPLY };
}
      ...
      while (active && queueRef.current.length > 0) {
        ...
        try {
          const calls = batch.map(id => ({
            ...
            abi: TOKEN_HTML_ABI,
            functionName: 'tokenHTML' as const,
...
const GridView: FC<{
  ...
  const cellSize = Math.max(
    80,
    Math.floor(((containerRef.current?.clientWidth || 1200) - GRID_GAP * (cols - 1)) / cols),
  );
...
// ─── Hover Preview (live mini-iframe of hovered parcel) ───────────────────

const HOVER_DEBOUNCE_MS = 200;
const PREVIEW_SIZE = 240;

/**
 * Debounced single-token tokenHTML fetcher with in-memory cache.
 * Keyed off `requestedId` — only fires once the user has settled on a parcel
 * for HOVER_DEBOUNCE_MS, so quick cursor sweeps don't trigger RPC calls.
 */
function useHoveredParcelHTML(requestedId: number | null) {
  ...
  // Debounce the requestedId → activeId
  useEffect(() => {
    if (requestedId === null) {
      setActiveId(null);
      return;
    }
    const t = setTimeout(() => setActiveId(requestedId), HOVER_DEBOUNCE_MS);
    return () => clearTimeout(t);
  }, [requestedId]);

  // Fetch HTML for activeId if not cached
  useEffect(() => {
    if (activeId === null) {
      setLoading(false);
      return;
    ...
    if (inflightRef.current.has(activeId)) {
      setLoading(true);
      return;
    }
    ...
    let cancelled = false;
    setLoading(true);
    inflightRef.current.add(activeId);
    ...
    publicClient
      .readContract({
        ...
        args: [BigInt(activeId)],
      })
      .then(html => {
        if (cancelled) return;
        cacheRef.current.set(activeId, html as string);
        forceUpdate(n => n + 1);
      })
      .catch(() => {
        /* ignore — leave skeleton up briefly */
      })
      .finally(() => {
  ...
  return {
    activeId,
    html: activeId !== null ? cacheRef.current.get(activeId) : undefined,
    loading,
  };
}

const HoverPreview: FC<{
  tokenId: number;
  html: string | undefined;
  loading: boolean;
  parcel: ParcelData | undefined;
  onClick: () => void;
}> = ({ tokenId, html, loading, parcel, onClick }) => {
    ...
    >
      ...
      <div
        style={{
          width: '100%',
          height: PREVIEW_SIZE,
          background: parcel?.color || '#0a0a14',
          position: 'relative',
        }}
      >
        {srcdoc ? (
          <iframe
            ...
            title={`Terraform #${tokenId} preview`}
            loading="lazy"
          />
        ) : (
          <div
          ...
          >
            <div
              style={{
                animation: loading ? 'parcelPreviewPulse 1.2s ease-in-out infinite' : undefined,
              }}
            >
              {loading ? `Loading #${tokenId}…` : `#${tokenId}`}
            </div>
          </div>
        )}
        <style>{`
          @keyframes parcelPreviewPulse {
            0%, 100% { opacity: 0.4; }
            50%      { opacity: 1; }
          }
        `}</style>
...
const HypercastleView: FC = () => {
  ...
  // Suppress hover preview when modal is open
  const previewRequestedId = openParcelId !== null ? null : hoveredId;
  const {
    activeId: previewId,
    html: previewHtml,
    loading: previewLoading,
  } = useHoveredParcelHTML(previewRequestedId);
  const previewParcel = useMemo(
    () => (previewId !== null ? parcels.find(p => p.tokenId === previewId) : undefined),
    [previewId, parcels],
  );

  const onClickParcel = useCallback((tokenId: number) => {
    setOpenParcelId(tokenId);
  }, []);

  const closeModal = useCallback(() => setOpenParcelId(null), []);
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 4, marginTop: 6 }}>
          <TabButton
            label="Terrain"
            active={viewMode === 'terrain'}
            onClick={() => setViewMode('terrain')}
          />
          <TabButton
            label="Lofi"
            active={viewMode === 'lofi'}
      ...
      {/* Bottom-left live NFT preview while hovering (3D modes only) */}
      {previewId !== null && viewMode !== 'grid' && (
        <HoverPreview
          tokenId={previewId}
          html={previewHtml}
          loading={previewLoading}
          parcel={previewParcel}
          onClick={() => setOpenParcelId(previewId)}
        />
      )}

      {/* Parcel modal (overlays current view) */}
      {openParcelId !== null && <ParcelModal tokenId={openParcelId} onClose={closeModal} />}
      ...
      {viewMode === 'grid' && (
        <GridView parcels={parcels} onClickParcel={onClickParcel} terrainData={terrainData} />
      )}
    </div>
  );
};
