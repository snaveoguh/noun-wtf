...
export function useGovernanceAction() {
  ...
  const execute = useCallback(async (action: GovernanceAction): Promise<string | null> => {
    setIsPending(true);
    setError(null);
      ...
      switch (action.type) {
        ...
        case 'PROMOTE': {
          ...
          if (sponsorSigs.length === 0) {
            throw new Error('No valid sponsor signatures to promote with');
          }
          hash = await writeContractAsync({
            abi: nounsGovernorAbi,
            address: nounsGovernorAddress[1] as Address,
