...
import type { EnsLookup } from './eventFormatters';
...
function hasExpandableContent(type: string, data: Record<string, unknown>): boolean {
  ...
  if (type === 'PROPOSAL_CREATED' || type === 'CANDIDATE_CREATED') {
    return !!(data.description && (data.description as string).length > 0);
  }
  // Votes/feedback with reasons
  if (type === 'VOTE' || type === 'PROPOSAL_FEEDBACK' || type === 'CANDIDATE_FEEDBACK' || type === 'CANDIDATE_SPONSORED') {
    return !!(data.reason && (data.reason as string).length > 80);
  }
  return false;
}
...
export default function ActivityEvent({ event, ensLookup }: Props) {
  const [expanded, setExpanded] = useState(false);
  const config = EVENT_TYPES[event.type];
  const color = config?.color || '#666';
  const label = config?.label || event.type;
  const description = formatEventDescription(event.type, event.data, ensLookup);
  ...
  const expandedContent = expanded
    ? (event.type === 'PROPOSAL_CREATED' || event.type === 'CANDIDATE_CREATED')
    ...
    >
      ...
      {expanded && expandedContent && (
        ...
        >
          ...
          {imageUrl && (
            <AsciiImage imageUrl={imageUrl} cols={70} />
          )}
