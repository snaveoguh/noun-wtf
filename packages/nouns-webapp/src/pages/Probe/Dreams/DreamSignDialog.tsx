...
const DreamSignDialog: FC<Props> = ({ candidate, onClose, onSigned }) => {
  ...
  const handleSign = async () => {
    ...
    if (!hasVotes) {
      toast.error('You need at least 1 Noun to sponsor');
      return;
    }
    ...
    try {
      ...
      await addSignature({
        args: [
          ...
          typedData.encodedProp,
          reason,
        ],
      });

      toast.success('Signature added! You are now sponsoring this dream.');
      onSigned();
      onClose();
    } catch (err) {
      ...
      <div className="mx-4 w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-6">
          <p className="text-sm text-gray-600">
            By sponsoring, you sign with your Noun
            {availableVotes && availableVotes > 1 ? 's' : ''} to support this dream
            becoming a DAO proposal. Your signature is valid until the expiration date.
          </p>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Why are you sponsoring this dream?"
              rows={3}
              className="border-border w-full rounded-lg border px-3 py-2 text-sm"
            />
          </div>
          ...
          {!hasVotes && (
            <p className="text-sm font-semibold text-red-500">
              You need at least 1 Noun to sponsor
            </p>
          )}

          {!address && (
            <p className="text-sm font-semibold text-red-500">Connect your wallet to sponsor</p>
          )}

          <Button
            onClick={handleSign}
            disabled={submitting || !address || !hasVotes}
            className="w-full"
          >
            {submitting ? 'Signing...' : 'Sign & Sponsor'}
          </Button>
        </div>
      </div>
    </div>
