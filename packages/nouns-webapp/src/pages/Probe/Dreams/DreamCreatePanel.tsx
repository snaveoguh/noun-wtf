...
/** Map layer key to the index in getNounData().parts (body=0, accessory=1, head=2, glasses=3) */
...
const DreamCreatePanel: FC<Props> = ({ onSave, onClose }) => {
  ...
  // Build composite SVG preview — swaps the custom layer if in upload mode
  const svgUri = useMemo(() => {
    try {
      ...
      if (mode === 'upload' && customEncoded) {
        const idx = LAYER_TO_PART_INDEX[customLayer];
        finalParts[idx] = customEncoded;
      }
          ...
          <div className="flex flex-1 flex-col gap-4">
            ...
            {mode === 'traits' ? (
              ...
              <div className="space-y-3">
                ...
                {/* Still show the other trait selectors (the non-custom ones) */}
            ...
            <Button onClick={handleSave} disabled={!title.trim()} className="mt-auto">
              Save Dream
            </Button>
          </div>
        </div>
