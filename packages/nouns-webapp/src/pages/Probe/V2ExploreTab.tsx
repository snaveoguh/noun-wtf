...
const V2ExploreTab: FC = () => {
  ...
  const nouns = useMemo<V2Noun[]>(() => {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      const owner = ownerRes?.status === 'success' ? String(ownerRes.result).toLowerCase() : '';
  ...
  // Settler / curated filters — indexer-backed via loadSettlerMaps (settler of N
  // = tx.from of AuctionSettled(N); curated is the +1 shift). Same source as the
  // /gamer profile; the static V2 snapshot is only the offline fallback.
  ...
  useEffect(() => {
    let cancelled = false;
    loadSettlerMaps('v2')
      .then(({ settlers, curated }) => {
        if (cancelled) return;
        setSettlerByNoun(settlers);
        setCuratedByNoun(curated);
      })
      .catch(() => undefined);
    return () => {
      cancelled = true;
  ...
  const settlerCounts = useMemo(() => countByAddress(settlerByNoun), [settlerByNoun]);
  const curatedCounts = useMemo(() => countByAddress(curatedByNoun), [curatedByNoun]);
  ...
  const filtered = useMemo(() => {
    ...
    if (settlerFilter !== '') {
      result = result.filter(
        n => (settlerByNoun[String(n.nounId)] ?? '').toLowerCase() === settlerFilter,
      );
    }
    if (curatedFilter !== '') {
      result = result.filter(
